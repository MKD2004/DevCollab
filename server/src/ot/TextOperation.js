...
class TextOperation {
  ...
  // Returns an operation that undoes this one when applied to the result
  // of applying this operation to `doc`.
  invert(doc) {
    let cursor = 0;
    const inverse = new TextOperation();
    for (const component of this.ops) {
      if (typeof component === 'number' && component > 0) {
        inverse.retain(component);
        cursor += component;
      } else if (typeof component === 'string') {
        inverse.delete(component.length);
      } else if (typeof component === 'number' && component < 0) {
        inverse.insert(doc.slice(cursor, cursor - component));
        cursor += -component;
      }
    }
    return inverse;
  ...
  // document as applying b then a'. Ties on insert-vs-insert favor `a`
  // (server can use a stable priority rule such as "first received wins").
  static transform(a, b) {
    ...
    for (;;) {
      ...
      if (typeof op1 === 'number' && op1 > 0 && typeof op2 === 'number' && op2 > 0) {
        ...
        // delete / delete — both remove the same underlying characters,
        // neither op needs to re-apply the deletion the other already did
        if (-op1 > -op2) {
          op1 -= op2;
          op2 = ops2[i2++];
        } else if (op1 === op2) {
